...
# === Upload Video with Progress ===
def upload_video(youtube, video_path, title, description, tags):
    title = re.sub(r'"', '', title).strip() or f"Untitled Video"
    description = re.sub(r'"', '', description).strip() or "No description provided."
    tags = [t.strip().lower() for t in tags.split(',') if t.strip()]
...
def main():
    ...
    for idx, row in df.iterrows():
        ...
        if not os.path.exists(video_path):
            print(f"⚠️ Skipping ID {row.get('ID', idx)}: video file not found")
            df.at[idx, "UploadStatus"] = "failed: file not found"
            continue

        if not title:
            print(f"⚠️ Skipping ID {row.get('ID', idx)}: title missing")
            df.at[idx, "UploadStatus"] = "failed: missing title"
            continue
